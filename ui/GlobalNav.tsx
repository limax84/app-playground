    ...
    <div className="fixed top-0 z-10 flex w-full flex-col border-b border-gray-800 bg-black lg:bottom-0 lg:z-auto lg:w-72 lg:border-r lg:border-gray-800">
      <div className="flex h-14 items-center py-4 px-4 lg:h-auto">
        <Link
          ...
          onClick={close}
        >
          <div className="h-7 w-7 group-hover:drop-shadow-logoDark">
            <JarvixLogo/>
          </div>

          <h3 className="font-semibold tracking-wide text-gray-400 group-hover:text-gray-50 mt-[-8px]">
            Jarvix Playground
          </h3>
        </Link>
      </div>
              ...
              <div key={section.name}>
                ...
                <div className="space-y-1">
                  {section.items?.map((item) => (
                    <GlobalNavItem key={item.slug} section={section} item={item} close={close}/>
                  ))}
                </div>
              </div>
            )
...
function GlobalNavItem({section, item, close}: {
  section: Item,
  item: Item
  close: () => false | void
}) {
